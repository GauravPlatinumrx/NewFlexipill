...
test('End-to-End Test for FlexiPill', async ({ page }) => {
  ...
  console.log('Stage Mobile End To End Test Cases Started');
  ...
  console.log('Clicked on See more results button');
  await contextPage.waitForTimeout(1000);
 ...
 // Wait for either "Address Details" or "Confirm Address" button to appear
await contextPage.waitForSelector('button:has-text("Address Details"), button:has-text("Confirm Address")', { timeout: 5000 });
await contextPage.waitForTimeout(1000);
if (await contextPage.getByRole('button', { name: 'Address Details' }).isVisible()) {
    ...
    await contextPage.waitForTimeout(1000);
    await contextPage.getByRole('button', { name: 'Male', exact: true }).click();
...
} else if (await contextPage.getByRole('button', { name: 'Confirm Address' }).isVisible()) {
    console.log('Confirming address...');
    await contextPage.getByRole('button', { name: 'Confirm Address' }).click();
} else {
    console.log('Neither "Address Details" nor "Confirm Address" button is found.');
}


  await contextPage.getByRole('button', { name: 'Proceed' }).click();

  await contextPage.locator('.AddToCartDropdown_arrow__pFEjt > svg').click();
  await contextPage.getByText('10', { exact: true }).click();
  await contextPage.getByText('COD (Cash on Delivery)').click();
  await contextPage.waitForTimeout(1000);
  ...
  await contextPage.waitForTimeout(1000);
  console.log('Order Reminder Confirmed');
  await contextPage.close();
  console.log("Stage Mobile End To End Test Cases Completed");
});
